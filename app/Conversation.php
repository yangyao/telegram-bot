...
class Conversation
{
     ...
     */
    public function __construct(Telegram $telegram, $user_id, $chat_id, $command = null)
    {
    ...
    protected function load()
    {
        ...
        $conversation = $this->telegram->getDatabaseHandler()->selectConversation($this->user_id, $this->chat_id, 1);
    ...
    protected function start()
    {
        if ($this->command
            && !$this->exists()
            && $this->telegram->getDatabaseHandler()->insertConversation(
                $this->user_id,
                $this->chat_id,
                $this->command
            )
    ...
    protected function updateStatus($status)
    {
        if ($this->exists()) {
            ...
            if ($this->telegram->getDatabaseHandler()->updateConversation($fields, $where)) {
                return true;
            }
        }
